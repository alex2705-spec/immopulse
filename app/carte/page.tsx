...
export default function CartePage() {
  ...
  // FIX: authLoading en dépendance pour re-exécuter quand le DOM est prêt
  useEffect(() => {
    ...
    <Link href="/prospection" style={{ padding: '8px 18px', borderRadius: 100, fontSize: 13, fontWeight: 500, border: '1.5px solid #E8EAED', color: '#6B7280', textDecoration: 'none' }}>Espace Prospection</Link>
      ...
      <div onClick={e => e.stopPropagation()} style={{ height: 60, flexShrink: 0, background: '#fff', borderBottom: '1px solid #E8EAED', display: 'flex', alignItems: 'center', gap: 8, padding: '0 24px', overflowX: 'auto', zIndex: 500 }}>
        {['A','B','C','D','E','F','G'].map(c => {
          ...
          return (
            <button key={c} onClick={() => toggleClasse(c)} style={{ width: 38, height: 38, borderRadius: 11, cursor: 'pointer', border: 'none', background: active ? GRAD : colors.bg, color: active ? '#fff' : colors.text, fontSize: 14, fontWeight: 700, flexShrink: 0, transform: active ? 'scale(1.08)' : 'scale(1)', transition: 'all .15s', outline: active ? 'none' : '1.5px solid #E8EAED' }}>{c}</button>
          )
        })}
        <div style={{ width: 1, height: 26, background: '#E8EAED', flexShrink: 0 }}/>
        <button onClick={() => setFilterEtage(!filterEtage)} style={{ padding: '9px 16px', borderRadius: 100, fontSize: 13, fontWeight: 500, border: 'none', outline: filterEtage ? 'none' : '1.5px solid #E8EAED', background: filterEtage ? GRAD : '#fff', color: filterEtage ? '#fff' : '#6B7280', cursor: 'pointer', flexShrink: 0, transition: 'all .15s' }}>
          Étage renseigné
        </button>
        <button onClick={() => setFilterFavoris(!filterFavoris)} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '9px 16px', borderRadius: 100, fontSize: 13, fontWeight: 600, border: filterFavoris ? `1.5px solid ${FAV_COLOR}` : '1.5px solid #E8EAED', background: filterFavoris ? FAV_BG : '#fff', color: filterFavoris ? FAV_COLOR : '#6B7280', cursor: 'pointer', flexShrink: 0, transition: 'all .15s' }}>
          <svg width="13" height="13" viewBox="0 0 16 16" fill={filterFavoris ? FAV_COLOR : 'none'} stroke={filterFavoris ? FAV_COLOR : '#9CA3AF'} strokeWidth="1.5"><path d="M8 13S2 9 2 5.5A3.5 3.5 0 0 1 8 3a3.5 3.5 0 0 1 6 2.5C14 9 8 13 8 13z"/></svg>
          Favoris
          {favoris.size > 0 && <span style={{ background: filterFavoris ? FAV_COLOR : '#E8EAED', color: filterFavoris ? '#fff' : '#6B7280', borderRadius: 100, padding: '1px 8px', fontSize: 11, fontWeight: 700 }}>{favoris.size}</span>}
        </button>
        <button ref={dateBtnRef}
          onClick={e => { e.stopPropagation(); showDatePicker ? setShowDatePicker(false) : openDatePicker() }}
          style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '9px 16px', borderRadius: 100, fontSize: 13, fontWeight: 500, border: 'none', outline: (isCustomMode || (filterDays !== null && filterDays !== 15)) ? 'none' : '1.5px solid #E8EAED', background: (isCustomMode || (filterDays !== null && filterDays !== 15)) ? GRAD : '#fff', color: (isCustomMode || (filterDays !== null && filterDays !== 15)) ? '#fff' : '#6B7280', cursor: 'pointer', flexShrink: 0, transition: 'all .15s' }}>
          <svg width="13" height="13" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5"><rect x="2" y="2" width="12" height="12" rx="2"/><path d="M5 1v3M11 1v3M2 6h12" strokeLinecap="round"/></svg>
          {isCustomMode ? `Personnalisé (${selectedDates.size} date${selectedDates.size > 1 ? 's' : ''})` : filterDays === 15 ? 'Période · 15j' : filterDays !== null ? `${filterDays} derniers jours` : 'Tout'}
          <svg width="13" height="13" viewBox="0 0 10 10" fill="none" stroke="currentColor" strokeWidth="1.5"><path d={showDatePicker ? "M2 6.5l3-3 3 3" : "M2 3.5l3 3 3-3"}/></svg>
        </button>
        {activeFiltresCount > 0 && (
          <button onClick={() => { setFilterClasse([]); setFilterEtage(false); setFilterFavoris(false); setFilterDays(15); setSelectedDates(new Set()) }}
            style={{ padding: '9px 16px', borderRadius: 100, fontSize: 13, fontWeight: 500, border: '1.5px solid #E8EAED', color: '#9CA3AF', background: '#fff', cursor: 'pointer', flexShrink: 0 }}>
            Réinitialiser ({activeFiltresCount})
          </button>
        )}
        <span style={{ fontSize: 13, color: '#9CA3AF', marginLeft: 'auto', flexShrink: 0 }}>{dpesFiltres.length} résultats</span>
      </div>
            ...
            {dpesFiltres.map(dpe => {
              ...
              return (
                <div key={dpe.id}
                  ...
                  onMouseEnter={() => setHoveredId(dpe.id)}
