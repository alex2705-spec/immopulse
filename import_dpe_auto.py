...
def get_codes_postaux():
    """Récupère tous les codes postaux distincts depuis user_access"""
    r = requests.get(
...
def transform(item, cp):
    ...
    return {
        ...
        "consommation_energie": si(item.get("conso_5_usages_ef")),
...
def main():
    # 1. Lire tous les codes postaux actifs depuis Supabase
    ...
    # 2. Scraper chaque code postal
    for cp in codes:
        print(f"\n📍 {cp}...")

        # Supprimer les anciens DPE
        requests.delete(
